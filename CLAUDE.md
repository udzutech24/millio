...
Перед любой задачей прочти [`~/.claude/ai-clone/INDEX.md`](~/.claude/ai-clone/INDEX.md) — цифровой профиль Алексея (ценности, стиль, принципы, обратная связь). Это базовый контекст, от которого работают все агенты.
...
**7. Главное окно = только оркестрация и решения.** Реализация, диагностика, верификация — в субагентах (Александр/Денис/millio-audit/Explore). Проверка чужой работы: требуй доказательства в ответе исполнителя (хвост build-лога, цифры тестов до/после, grep-чек) и точечно перепроверяй 1–2 места — НЕ перечитывай все диффы и не перегоняй сборку в главном окне.

**8. Одна крупная задача = одна сессия.** Пришла вторая независимая задача — заверши текущую (коммит или handoff в `progress/`), предложи новый чат. Два параллельных расследования в одном окне = перегрев.

**9. Тяжёлые команды — глуши вывод сразу.** `xcodebuild … -quiet 2>&1 | tail -20` (никогда без фильтра); `git diff --stat` сначала, полный дифф — только 1–2 файлов; субагенту в брифинге — жёсткий лимит ответа (≤200–250 слов, file:line вместо цитат кода).
...
## Автономные многофазные сессии (владелец недоступен)

Когда план на несколько под-фаз выполняется без постоянного присутствия владельца (day/night run, «уйду на N часов») — поверх обычного Workflow добавляется:

- **Fable = план + верификация, opus (Александр) = код.** Перед стартом — один Fable-проход: ревью плана (не устарели ли research file:line, нет ли пропусков в рисках/инвариантах) + стресс-тест по всем под-фазам сразу (10 причин провала, акцент на самые рискованные/L-фазы). После КАЖДОГО гейта — отдельный Fable-верификатор адверсариально перепроверяет критерии гейта сам (grep/тест-хвост/инварианты), не пересказ само-отчёта исполнителя. Коммит — только после независимого подтверждения.
- **Стоп на первом красном/неподтверждённом гейте.** Не чинить в обход, не начинать следующую под-фазу поверх сломанной. Коммитить только зелёное+подтверждённое; остальное — в handoff с гипотезой причины.
- **Device-тест без владельца — PENDING, не блокер.** Если гейт требует реального устройства, а владельца нет — симуляторный эквивалент + явная пометка «нужна проверка на устройстве» в `status.json`/handoff; это не красный гейт, работа продолжается дальше по плану.
- **Ментор-дисциплина не выключается автономностью.** Если исполнитель видит, что шаг плана — не лучший путь или решение можно проще — фиксирует это явно в handoff (альтернатива + почему), даже продолжая по плану как согласовано.
- **Push/merge/deploy — только с владельцем лично**, даже если по плану это конец дня/ночи (соответствует общему правилу деплоя из корневого `CLAUDE.md`). В handoff — готовая сводка для решения, не сам pushed/merged результат.

## Git-дисциплина

### Модель веток (закреплена 07.09.2026)

**Ровно две долгоживущие ветки. Третьей не заводить.**

| Ветка | Что это | Кто двигает |
|---|---|---|
| `develop` | Вся разработка. Дефолтная ветка (`origin/HEAD`). Отсюда собираются TestFlight-билды | мерж feature-веток |
| `prod` | Только то, что реально выкачено в App Store | мерж из `develop` на релизе + тег `v<версия>-build<N>` |

- `master` **удалён** 07.09.2026 — окаменелость GitLab-эпохи (отставал на 793 коммита, ноль уникальных коммитов). Не воскрешать.
- Feature-ветка живёт **от `develop`**, а не цепочкой от другой feature-ветки. Цепочки `Ф1→Ф2→Ф3` — источник путаных мержей; если предыдущая фаза уже в `develop`, ветвись от `develop`.
- Каждая задача = `feature/<task>`. Коммит после каждого пройденного gate.
- НЕ пушить в `develop`/`prod` напрямую. Мерж — только после device-проверки владельцем и его явного «да».
- Remote: GitHub `udzutech24/millio`. GitLab отключён.

### Гигиена веток (обязательна, иначе снова 50 веток)

- **Смержил → удали ветку сразу**, локально и на remote (`git push origin --delete <b>`). Ветка, целиком лежащая внутри `develop`, — мусор, а не история: история уже в `develop`.
- **Ветка протухла** (отстаёт от `develop` на 100+ коммитов и не нужна) → не ребейзить, а **сохранить тегом** `archive/<имя>` и удалить ветку. Коммиты остаются доступны навсегда, список веток чист.
- **Раз в месяц** — ревизия: `git branch -r --merged origin/develop` (всё это под снос) и `git branch -r --no-merged origin/develop` (по каждой решить: доделать / архив-тег / снести).
- Worktree — только внутри проекта (`millio-dev/worktrees/`), и **сносить сразу после мержа ветки**: забытый worktree держит ветку и мешает её удалить.

**Итог уборки 07.09.2026:** remote 50 веток → 4 (`develop`, `prod`, `feature/account-screens-cash`, `feature/planned-operations-applied-notice`), локально 22 → 7, worktree 3 → 1. Ничего не потеряно: 42 ветки были целиком внутри `develop`, остальные сохранены тегами `archive/*`. `prod` переведён на релиз 2.0(2) = тег `v2.0-build2`.
