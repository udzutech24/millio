...
final class AccountsCoreService {
    ...
    /// Помечает счёт удалённым (`deletedAt`), НЕ снося события/снапшоты. История графика до даты
    /// удаления остаётся неизменной (Q1: «история сохраняется»), в отличие от каскадного
    /// `physicallyDelete`, который переписывал прошлое. Счёт исчезает из всех UI-списков: главные
    /// списки фильтруют через `participates(on:)` (deletedAt даёт участие 0 «сегодня»), а списки
    /// архива — предикатом `deletedAt == nil`. В тоталах вклад считается только для дат ДО удаления.
    /// Ноги переводов НЕ конвертируем (в отличие от `physicallyDelete`): обе ноги остаются в БД, так
