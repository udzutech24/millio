...
- показ реальных исторических значений по счетам на конец выбранного дня (через расчёт динамики);
- ручная корректировка доступна в режиме редактирования для любой строки за выбранную дату;
- ввод суммы выполняется через форматтер (`AmountInputFormatter`) с группировкой разрядов прямо во время ввода;
- сохранение изменений выполняется явной кнопкой `Сохранить` (зелёная панель внизу);
- удаление значения на дату возвращает строку к расчетному историческому значению;
...
- для живых счетов источник значения — исторический расчёт (`FinanceDynamicsViewModel.calculateBalanceAtDate`) на конец дня;
- snapshot-store хранит ручные override-значения по дням и используется в динамике/графике при расчете баланса на соответствующую дату;
- если для строки есть сохраненное значение на дату в snapshot-store, оно имеет приоритет над расчетным (как в audit-экране, так и в Dynamics);
- после сохранения/удаления значения публикуется событие `FinanceEvent.auditSnapshotsUpdated`, чтобы зависимые экраны перезагружали данные;
