...
- snapshot-store используется как fallback только для неизвестных/удалённых счетов;
- если для строки есть сохраненное значение на дату в snapshot-store, оно имеет приоритет над расчетным;
