...
- Backup хранится в **CloudKit Private DB** (`AppBackup` snapshot history + legacy `latest_backup`) внутри iCloud-аккаунта пользователя, а не в публичном хранилище Millio
- Backup **не требует Sign in with Apple в Millio**: для него нужен доступный iCloud/CloudKit на устройстве; backend auth и backup — разные контуры
...
- Автобэкап сохраняет несколько auto-snapshot версий `AppBackup` с retention и обновляется не чаще одного раза в 3 дня при фоновой активности приложения; `latest_backup` поддерживается как legacy fallback
...
- Виджет конвертера показывает 3 верхние избранные валюты в пересчете к основной валюте, поддерживает `small/medium/large`, и имеет быстрые deep-link действия: открыть конвертер, добавить расход, добавить доход.
...
- После добавления новых `CKRecord` типов или полей schema нужно вручную выкатить из CloudKit `Development` в `Production`, иначе в TestFlight/App Store backup упадет с ошибкой вида `Cannot create new type ... in production schema` или `CloudKit production schema is missing record type ...`
