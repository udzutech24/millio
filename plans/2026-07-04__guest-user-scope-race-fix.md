...
**Дата:** 2026-07-04 · **Статус: ГОТОВ К /stress-test, кроме одного открытого решения** (Track A A1–A4 ✅,
Track B B1–B2 ✅, Track C C1–C3 ✅, Track D D1 ✅, Находка 1 ✅ закрыта, Находка 2 🟡 ждёт решения владельца)
...
- 2026-07-05: **Сессия «добить Track B2 + закрыть находки».** Верификация показала, что Track B2 был
  полностью реализован ещё в ночной сессии (коммиты e9a812d/466805a/4dacba9/56402e1 + hardening
  dc440ed — name-fallback групп и watchdog, обе митигации B1b №4/№7, добавлены ПОСЛЕ журнальной записи
  выше) — под-фаз, требующих продолжения, не найдено. Работа сессии:
  - **Находка 1 (race guest→user) — ЗАКРЫТА, кода не потребовалось.** Перечитан `millioApp.swift`
    (rebindDataScope :473-542, reconcileScopeIfNeeded :420-470, RootSceneIdentity :48-51) и
    `RootTabView.swift` (:330-332). Диагноз в `progress/accounts-core-rebuild-handoff.md` §Сим-проверка
    описывал состояние ДО Track A (коммит написан в 22:28, Track A landed 23:04-23:30 того же вечера) —
    текст не был обновлён после фикса и вводил в заблуждение постановку задачи. Фактически:
    `scopeIdentityToken` бампится СРАЗУ после свопа `activeModelContainer` (millioApp.swift:534) →
    `.id(RootSceneIdentity(...))` пересоздаёт весь `RootTabView` → `ensureFinanceViewModel()` создаёт
    VM заново на новом `modelContext` — ровно требуемый класс фикса, уже в A1+A2. Историческая
    дупликация legacy-счетов (накопленная ДО фикса) лечится Track B `reconcileScopeIfNeeded` —
    идемпотентный merge, проверенный на копиях РЕАЛЬНЫХ сторов (B2d). Остаётся только ручная проверка
    с реальным Apple ID (недоступна в этой среде, не новый блокер — уже была помечена в A4).
    Handoff-текст переписан, чтобы отражать реальное состояние (mentor-lesson: закрывать находки в
    документации сразу после факта фикса, не оставлять «found: broken» рядом с уже слитым коммитом фикса).
  - **Находка 2 (V4 in-place) — РЕШЕНИЕ ЭСКАЛИРОВАНО владельцу, код НЕ менялся.** Разобраны два
    варианта (детали см. отчёт сессии владельцу): (A) заморозить V4.models как финальный набор +
    guard-тест, фиксирующий список типов (дешёво, но полагается на факт «V4 никогда не уходил во
    внешний TestFlight/App Store» — этой веткой подтверждено, но неявное допущение); (B) честный V5 —
    вернуть V4.models к исходному набору (без HistoricalAssetPrice), перенести HistoricalAssetPrice в
    новый V5 через lightweight-стадию V4→V5, `AppSchemaCurrent = AppSchemaV5`. Рекомендация ментора: (B),
    т.к. не полагается на допущения о дистрибуции и это единственный момент, когда правка стоит дёшево
    (до мержа в develop). Ждём явного «да» — правило CLAUDE.md §7 (стресс-тест перед изменением схемы/
    данных пользователя) не отменяется guard phrase.
  - **Мелочь — починена.** `accounts_core.debug.seed_success` (en/ru/zh-Hans) писал «10 счетов»,
    факт — 12 (сеятель создаёт 2 вклада + 2 валютных + 2 акции + 1 loan + 1 debt + 3 карты + 1 apartment
    = 12, уже покрыто тестом `accounts.count == 12`). Поправлен текст во всех 3 локалях; переименован
    вводящий в заблуждение тест `seedCreatesTenRealAccounts` → `seedCreatesTwelveRealAccounts`.
  - **Тест-гейт:** build ✅ 0 ошибок; полный `millioTests` прогон — см. итог ниже (сверка с baseline).
  - **Не тронуто:** Track C/D, Cashflow-категории (параллельный процесс), физический снос легаси (6b) —
    вне скоупа сессии, guard phrase не давалась.
