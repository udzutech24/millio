...
**Побочная находка при гейте (не мой скоуп, флагирую честно):** `GroupsMigratorTests` (Ф1.5, файл не тронут в Ф2/Ф2b) при прогоне через `xcresulttool` (ground truth, не текстовый summary xcodebuild) стабильно падает с «Test crashed with signal trap» — `makeStack()` возвращает `container.mainContext`, не удерживая сам `ModelContainer` (нет `retainedContainers`), из-за чего in-memory store может быть освобождён сразу после return (use-after-free). Тот же класс бага я поймал и исправил в СВОЁМ новом `LegacyMigrationOrderingTests` при первой попытке. Важно: прежний self-audit Ф1.5 «8/8 ✔» и мой собственный ранний self-audit Ф2 «0 новых красных, wrapper-артефакт» ОШИБОЧНЫ в части `GroupsMigratorTests` — консольный текстовый summary xcodebuild («✔ Suite … passed») ненадёжен и может показывать ложный ✔ при реальном крэше; впредь для крэш-подозрительных сьютов проверять через `xcrun xcresulttool get test-results summary --path <bundle>` (JSON `passedTests/failedTests`), не через grep по тексту. Это НЕ регрессия Ф2/Ф2b (`GroupsMigratorTests.swift`/`GroupsMigrator.swift` не в диффе) — существующий баг Ф1.5, нужен отдельный маленький фикс (добавить `retainedContainers`, как в остальных тестах файла), не входит в этот коммит.

### 🟠 Фаза 2b — Остаток single-world read-слоя (escalated, требует «1b» + device stress-test)
Вынесено из Ф2 как доказанно-рискованное (см. отклонения Ф2). Пункты: (а) снос `mergingNewCoreSeries` + перевод аггрегированного графика Динамики на чистый `seriesBetween` — вместе с портом per-account chart-modes (byAccounts/singleAccount) и скоуп-заголовка на ядро («1b»); (б) унификация `FinanceRows`/`orderedAccounts(for:)`→`[Account]` со сносом `FinanceAccountRow`/`getAccountInfo`-каскада (координировать с Ф4); (в) перевод `VM.calculateGroupTotal` на core-only (после сноса легаси Ф4/5 — само схлопнётся). AC: график/список single-world, protected Dynamics-тесты мигрированы на core, device-проверка истории графика.
