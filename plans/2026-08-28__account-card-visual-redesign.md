...
Что дальше:
1. Визуальный мокап ПЕРЕД кодом — прогони /design или /frontend-design (посмотри,
   что подходит по формату — canvas-мокап экрана «Счета» списком + hero-карточки
   детального экрана), используя решения владельца: только градиенты/паттерны
   (без фото), монограмма+цвет вместо банк-логотипов, PRO-бейдж на галерее.
   Референс стиля владелец давал — Bybit "My cards" (яркая карточка на весь верх).
2. Покажи мокап + текстовое описание функционала (что кликабельно, что открывает
   галерею, как выглядит Free vs PRO) владельцу, собери правки ДО кода.
3. Только после одобрения дизайна — переходи к Ф0 (схема AccountAppearance V11):
...
Развилки/ждём: одобрения мокапа дизайна и явного «да» на риск схемы в Ф0 — без них
код не начинать (Guard phrase + правило стресс-теста CLAUDE.md).
```
