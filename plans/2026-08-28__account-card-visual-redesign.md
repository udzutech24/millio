...
**Дата:** 2026-08-28
**Статус:** В РАБОТЕ (Ф0 смержена в `develop` 1ba9c2e; Ф1+Ф1b в ветке `feature/account-card-row-redesign`;
Ф2+Ф3 в ветке `feature/account-appearance-gallery-hero` — обе НЕ смержены и НЕ запушены; осталась Ф4)
...
### [x] Ф2. Каталог пресетов + галерея выбора (PRO-гейт)
...
**Факт реализации (2026-08-29, ветка `feature/account-appearance-gallery-hero`, `bfb412f`, НЕ смержена):**
- `UI/Services/Finances/Icons/AccountAppearancePreset.swift` — 12 пресетов, hex-пары взяты из
  градиентов `AppColors`. Hex строками, а не `[Color]`: бейдж строки красится ОДНИМ акцентом,
  из `Color` его обратно не достать. В БД лежит только `rawValue`.
- Вкладка «Дизайн» — внутри `AccountIconPickerSheet` (новый экран не заводился). Появляется только
  там, где есть куда писать пресет: строка core-счёта (`NewCoreAccountRow`) И строка легаси-счёта
  (`FinanceRows`), то есть **все 4 типа продуктов**. У иконки группы и формы создания счёта вкладки
  нет — им негде хранить выбор.
- Гейт — только `EntitlementPolicy.canUseAccountAppearanceGallery`. Free видит витрину пресетов
  (что он получит) и ведёт на paywall через штатный `premiumUpsellAlert`; иконка/цвет остаются
  бесплатны — они были бесплатны до V11, отбирать их нельзя.
- Дизайн и ручной цвет **взаимоисключающи** в пикере; порядок в резолвере
  (`tintHex` → акцент пресета → легаси → дефолт) нужен на случай бэкапа чужой/будущей версии,
  где заполнены оба поля.
- `AccountAppearanceSnapshot`/`AccountAppearanceStore.setAppearance`/`FinanceViewModel.saveAppearance`
  получили `presetRaw`; бэкап-импортёр Ф0 это поле уже вёз.
- 17 ключей локализации (ru/en/zh-Hans), вставлены точечно (файл целиком не переписан).
- Тесты: `millioTests/UI/Services/Finances/AccountAppearancePresetTests.swift` — 12/12 зелёных,
  включая неизвестный `presetRaw` и локализацию всех названий.
- ⚠️ **Открыто:** device-проверка владельцем. Контраст пресетов в тёмной теме проверялся кодом
  (общее затемнение 22% под контентом hero), но не глазами на устройстве.

---

### [x] Ф3. Hero-карточка детального экрана
...
**Факт реализации (2026-08-29, ветка `feature/account-appearance-gallery-hero`, `b6243d5`, НЕ смержена):**
- `UI/Services/Finances/AccountsCore/Hero/AccountHeroCardView.swift` — `AccountHeroPresentation`
  (value, собирается ТЕМ ЖЕ `resolvedAppearance`, что строка списка) + единственная вёрстка шапки.
  Подложка: градиент выбранного дизайна (Ф2), а без дизайна — акцент счёта, тот же, что красит
  бейдж в списке, чтобы деталка и список не расходились по цвету.
- **Сумма hero** = `AccountTotalsContribution.signedValue(подтверждённый баланс)` — буквально то же
  выражение, что `FinanceViewModel.newCoreBalanceToday`. У кредитки это долг со знаком минус, у
  вклада — confirmed-путь, а не сырой реплей. Третьего определения баланса не появилось.
- Сняты: `standardHeader` и `stockHero` (`AccountDetailView`), блок идентичности в
  `CreditCardDetailSection` и `DebitCardDetailSection`, строка баланса в `DepositDetailSection`.
  Их уникальное содержимое переехало в hero (`heroDetailLines` / `heroBadges` / `heroSubtitle`):
  банк+last4, ставки и сроки, направление долга, заметка, нереализованный P/L, статусы «архив» /
  «не в тотале» / свежесть цены.
- **Легаси-счёт (`Card`/`Credit`/`Investment`) своего детального экрана не имеет** — тап по строке
  ведёт в `FinanceDynamicsView` (`.showAccountDynamics`). Поэтому hero встал и туда, в
  single-account режиме: иначе редизайн опять закончился бы на строке списка, как в провале Ф1.
- `AccountKind.localizedTitle` поднят из приватной копии в `ArchivedAccountsView` — минус дубль.
- Тесты: `millioTests/UI/Services/Finances/AccountHeroPresentationTests.swift` — 9/9 зелёных,
  ключевой — инвариант «hero == строка списка» и «вклад: confirmed ≠ сырой реплей».

**Отступления от буквы плана (осознанные):**
1. **Недвижимость сохраняет собственную шапку.** Её hero — фото-обложка объекта (баланса на ней
   нет), а пользовательские фото прямо вынесены из скоупа решением №2. Заменить её градиентом =
   удалить фотографии с экрана. Generic-hero над ней не ставится, чтобы не было двух шапок подряд.
2. **Per-type секции сохранили свои метрики.** Убрана только дублирующая ИДЕНТИЧНОСТЬ (имя, баланс,
   статусы), а не карточки «доступный лимит / утилизация / начислено %». Буквальное «секции не
   рисуют собственных шапок» при удалении метрик означало бы потерю информации.
3. Кредитка на hero показывает сводку (доступно · дата платежа), детальные строки остались в
   секции — это сводка, а не второе определение.

**Открыто:** device-проверка владельцем по каждому типу (наличные/карта/кредитка/вклад/акция/
легаси-счёт); raw RU-литералы внутри `CreditCardDetailSection` («Платёж просрочен», «Available
limit») — долг ДО Ф3, не трогал.
...
| 2026-08-29 | Ф1b подтверждена владельцем на устройстве (цветные монограммы на легаси, редактор оформления в контекстном меню). Ф2 и Ф3 реализованы в ветке `feature/account-appearance-gallery-hero` от `feature/account-card-row-redesign` (`bfb412f`, `b6243d5`). Гейт: `BUILD SUCCEEDED`; полный сьют 26 red против baseline 25 red — 3 «новых» (`FinanceDynamicsEndpointSemanticsTests` ×2, `FinanceViewModelTotalsRaceTests`) прогнаны точечно на обеих ветках и ЗЕЛЁНЫЕ, это флейки полного прогона, не регрессия; 2 красных из baseline при этом позеленели. Ни merge, ни push не делались. |
