...
Дата: 2026-09-07 · Статус: **В РАБОТЕ** — Ф3.1–Ф3.5 закрыты, осталась Ф3.6 (device-проверка
владельцем и решение о мерже)
...
## Ф3.4 — UI: платёж через план `[x]` (M, ~0,4 сессии)
...
### 2026-09-07 · сессия B: Ф3.4 UI платежа через план

**Что сделано.** «Внести платёж» = `LoanPlannedPaymentScheduler.applyNextPlannedPayment`
(`AccountDetailView+Sheets.swift`, `recordLoanPayment`) — независимой факт-транзакции кнопка больше
не создаёт. Кнопка появляется, только если план реально лежит в Cashflow
(`AccountDetailActions.loanActionItems`). В разбивке платежа — строка «Следующий платёж» с датой и
подписью «Запланирован в Cashflow · <шаг>» (`LoanDetailSection`, `LoanBreakdownRow.caption`); та же
подпись в шапке листа подтверждения. Лист досрочки получил шаг подтверждения изменения плана
(`LoanPrepaymentSheet.planChangeNotice`), сбрасываемый при смене суммы или сценария.
`LoanDetailPresentation` получила `plannedPaymentDate` и `plannedRecurrenceTitle`.

**Решения, отличные от буквы плана:**
- **Дата плана — состояние (`loanPlannedPaymentDate`), а не расчёт внутри `loanPresentation`:**
  витрина пересчитывается на каждый проход `body`, а выборка плана ходит в стор. Обновляется одной
  точкой `refreshLoanState()` (договор + план вместе), заменившей три разрозненных перечитывания.
- **Подтверждение плана — шаг внутри того же листа, а не второй лист поверх первого:** правило
  «под большим пальцем» и без стека модалок; цифры берутся из уже показанной таблицы «что
  изменится», второго расчёта не заводится.
- **`plannedPaymentDate` отделён от `nextPaymentDate`:** график знает дату всегда, а применять
  можно только то, что реально лежит в Cashflow — на этом различии и держится скрытие кнопки.
- **`applyNextPlannedPayment` сначала чинит план, если его нет** (сбой синхронизации, старый
  бэкап), и только потом применяет: заводить «просто платёж» в обход плана значило бы вернуть
  второй путь записи.

**Проверить на устройстве (владелец):** деталка кредита — строка «Следующий платёж ·
Запланирован в Cashflow»; кнопка «Внести платёж» и её исчезновение на закрытом кредите; лист
подтверждения платежа (дата, сумма, тело/проценты, пометка о плане); операция в «Предстоящих»
Cashflow и её пропажа после платежа; шаг «План платежей изменится» в листе досрочки и кнопка
«Назад»; лист досрочки — обе строки с тегами экономии и
