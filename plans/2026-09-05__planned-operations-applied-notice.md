...
Статус: **НЕ НАЧАТ** (ждёт явного «Реализуй фазу 0 по плану»).
...
## Фаза 0 — журнал непоказанных применений `[ ]`

Новый файл `millio/Core/Cashflow/AppliedPlannedNoticeStore.swift`.
...
**Гейт Ф0:** unit-тесты — append→takeDigest возвращает данные; повторный takeDigest пуст;
две разные scope-строки не видят записей друг друга; при 300 append счётчик = 300, деталей 50.
