...
## Состояние
- ✅ **СМЕРЖЕНО в develop 2026-07-05** (merge-коммит 258295d; перед ним ff-мержи фиксов дедупа:
  9799b82 cashback, 8e49209 budget-limit + спека ReminderMeta a08ec1b). Владелец явно решил
  мержить БЕЗ ручных проверок на устройстве («давай без ручных проверок», 2026-07-05) —
  чек-лист ручных проверок (login/logout, force-signout, fresh install+iCloud, конвертация
  легаси) остаётся актуальным для прогона ПОСЛЕ мержа при первой возможности.
- Конфликт мержа был один: DataIntegrityCleanerMigrationTests.swift — взята сторона develop
  (per-scope идемпотентность из 729eec5/7b9c8bb). Прицельный тест-гейт после мержа — см. итог сессии.
- Решение 6b (снос легаси) — по-прежнему ОТКРЫТО, за владельцем. Не запушено.
- Ветка **`feature/accounts-core`** сохранена; история ниже актуальна на момент до мержа.
