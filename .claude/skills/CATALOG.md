...
### 7. millio-forensics ⭐ Project-specific
**Что даёт:** Расследование расхождений цифр (баланс/тотал/график не сходится между экранами) по РЕАЛЬНОМУ бэкапу пользователя, а не по догадкам из кода. Умеет расшифровать `.milliobackup` (MBKP-формат, LZFSE + опционально AES-GCM, пароль спрашивается локально через `getpass`), провести раскопку JSON-дампа через python3/jq без сжигания контекста, сверить возраст среза с симптомом, и держит каталог известных классов багов Millio (двойное определение величины, легаси-двойник сущности, кэш-срез vs живые данные, не вычищенный хвост расписания, невалидированный кэш формулы).  
**Когда нужен:** «цифра на экране A не сходится с B», «баланс неправильный», «откуда эта сумма», «сними дамп и проверь по бэкапу», любое расследование дублей/кэш-сбоев/странной арифметики.  
**Расположение:** `~/.claude/skills/millio-forensics/` (симлинк в проекте)  
**ID:** `millio-forensics`
